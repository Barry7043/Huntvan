  ...
  app.post('/popup', select_comment,(req,res)=>{
    ...
    pool.query(
      'select * from postdata WHERE id = $1', [postid], (err,results)=>{
        ...
        us2 = [];
        ob = {'postid':postid,'comment_email':comment_email,};
        us2.push(ob);
        res.json(us2);
    ...
    //寻找是否已经注册
      pool.query(
        `SELECT * FROM users WHERE email = $1`, [email], (err, results) => {
          ...
          us1 = [];
          ob = {'name':name,'email':email, 'password':password,'password2':password2};
          us1.push(ob);
          res.json(us1);
...
//login
  app.post('/login', (req,res)=>{
    ...
    pool.query('SELECT * FROM users WHERE email = $1',[email], (err,results)=>{
      ...
      us = [];
      ob = {'email':email, 'password':password};
      us.push(ob);
      res.json(us);
...
//储存进db
  ...
  //text,budget,place部分
    let {username, title, post_text, place, budget, email, latt, lngg, icon, img_name} = req.body;
  ...
  //  if (req.file){
      ...
      pool.query(postsql,postsql_params,(err,results)=>{
        ...
        //console.log(results);
        info = [];
        ob = {'username':postsql_params[0], 'title': postsql_params[3], 'post_text':postsql_params[1],
              'place':postsql_params[4],'budget':postsql_params[5],'email':postsql_params[6],
              'icon':postsql_params[7]
            };
        info.push(ob);
        res.json(info);
...
app.post('/commend', (req,res)=>{
  ...
  pool.query(sql,sqlparams, (err,results)=>{
    ...
    info1 = [];
    ob = {'postid':postid,'comment_email':comment_email,'countheart':countheart,'likes':likes};
    info1.push(ob);
    res.json(info1);
...
app.post('/follow', (req,res)=>{
  ...
  //if (followornot == 1){
    pool.query(`INSERT INTO follower (uemail,femail,username) VALUES ($1,$2,$3)`, [uemail,femail,username], (err,results)=>{
      ...
      info3 = [];
      ob = {'femail':femail,'uemail':uemail,'username':username};
      info3.push(ob);
      res.json(info3);
